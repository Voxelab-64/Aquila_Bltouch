...
// #define BABY_Z_VAR TERN(HAS_LEVELING, probe.offset.z, dwin_zoffset)

// #define BABY_Z_VAR zprobe_zoffset
...
inline void Prepare_Item_Bltouch(const uint8_t row){
  if(HMI_flag.language_flag)
   ...
   // DWIN_ICON_Show(ICON1,28,LBLX,MBASE(row));
      DWIN_Frame_AreaCopy(1,260,250,390,266,LBLX,MBASE(row));
  else
    //DWIN_Frame_AreaCopy(3,0,2,48,20,LBLX, MBASE(row));
...
inline void Draw_Control_Menu() {
  ...
  if (HMI_flag.language_flag) {
     DWIN_Frame_AreaCopy(1,240,2,270,16,285,12); //??
  }
  else
  {
  ...
  if (PVISI(4)) Prepare_Item_Home(PSCROL(4));       // Auto Home
   if (PVISI(5)) Prepare_Item_Offset(PSCROL(5));     // Z-Offset
...
inline void Draw_Prepare_Menu() {
  ...
  if (HMI_flag.language_flag) {
     DWIN_Frame_AreaCopy(1,208,2,238,16,285,12); //??
...
void HMI_Zoffset(void) {
  ...
  if (encoder_diffState != ENCODER_DIFF_NO) {
    ...
    if (HMI_ValueStruct.show_mode == -4)
      show_plus_or_minus(font8x16, White,Select_Color, 2, 2, 400, MBASE(5 + MROWS - index_control), HMI_ValueStruct.offset_value);
    else
...
void update_variable(void) {
  ...
  if (checkkey == TemperatureID) {
    ...
    if (last_zoffset != zprobe_zoffset) {
      show_plus_or_minus(MENU_FONT,Black, Background_black, 2, 2, 160, 220, zprobe_zoffset * 100);
      last_zoffset = zprobe_zoffset;
    }
  #endif
  }
...
inline void Draw_Move_Menu() {
 ...
 // 
  if (HMI_flag.language_flag) {
    DWIN_Frame_AreaCopy(1,156,50,200,66,285,12);//???
...
void Draw_matic_Menu(){
      ...
      else
      {
        ...
        DWIN_ICON_Show(ICON,25,155,225);                 //????
        DWIN_Frame_AreaCopy(1, 340, 166, 400,180, LBLX, MBASE(4)-10);
      }
  DWIN_Draw_Line(0X0000,140,210,450,210);
  DWIN_Draw_Rectangle(1, Red_Color, 140, 210,150,250);
  ...
  if (encoder_diffState == ENCODER_DIFF_CW) {
    ...
    { 
      ...
      show_plus_or_minus(font8x16, Black, Background_black, 2, 2, 400, MBASE(5 + MROWS - index_control), HMI_ValueStruct.offset_value);
		...
		else if (index_control == 6) Prepare_Item_PLA(MROWS);
